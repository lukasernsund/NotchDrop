...
struct QuickLookPreviewView: NSViewRepresentable {
    ...
    var aspectRatio: CGFloat  // The aspect ratio of the content
...
struct ClipboardView: View {
    ...
    private var isNotchOpenBinding: Binding<Bool> {
        Binding(
            get: { self.vm.notchVisible },
            set: { newValue in
                self.vm.notchVisible = newValue
                if !newValue {
    ...
    var searchBar: some View {
        ExpandingSearchBar(searchText: $searchText, isExpanded: $isSearchExpanded)
    }
        ...
        GeometryReader { geometry in
            VStack(spacing: 20) {
                ...
                QuickLookPreviewView(
                    url: item.storageURL, aspectRatio: calculateAspectRatio(for: item)
                )
                .frame(
                    width: geometry.size.width,
                    height: calculatePreviewHeight(for: item, in: geometry)
                )
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        metadataRow(
                            icon: "app.badge", title: "Source", value: item.sourceApp ?? "Unknown")
                        ...
                        if item.itemType == .color, let color = Color(hex: item.previewText) {
                            ...
                            metadataRow(
                                icon: "eyedropper.full", title: "RGB", value: color.rgbString)
                            metadataRow(
                                icon: "eyedropper.halffull", title: "HSL", value: color.hslString)
                        }
    ...
    func calculateAspectRatio(for item: Clipboard.ClipboardItem) -> CGFloat {
    ...
    func calculatePreviewHeight(for item: Clipboard.ClipboardItem, in geometry: GeometryProxy)
        -> CGFloat
    {
        let aspectRatio = calculateAspectRatio(for: item)
        let calculatedHeight = geometry.size.width / aspectRatio
        return min(calculatedHeight, geometry.size.width)  // Ensure height doesn't exceed width
    }
